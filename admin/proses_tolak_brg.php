...
if ($b['sisa_approval'] != 0 ) {
	$l = mysqli_query($conn,"UPDATE pengadaan_barang SET status = 'ditolak', tgl_sah = '$tgl_sah', disahkan = '$username' WHERE id_pbarang = '$id_pbarang'") or die(mysqli_error($conn));
	
	echo "<script>alert('Penyetujuan Berhasil ditolak')</script>";
	
}else{
	echo "<script>alert('Penyetujuan Berhasil Tapi Mohon Tunggu Approvel lain untuk menolak')</script>";
}
